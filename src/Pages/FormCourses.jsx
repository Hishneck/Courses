...
const FormCourses = ()=> {
    ...
    return(
        ...
        >
            <Box component="form" sx={{width: 700,}}> 
                <CardContent sx={{display: "flex", flexDirection: "column", margin: 3, gap:2, alignItems:'center'}}>
                    <TextField 
                      id="standard-basic" 
                      label="Название:" 
                      variant="standard" 
                    ...
                    <TextField 
                      ...
                      type="text" 
                      value={newDesc} 
                      onChange={(e)=> {setDesc(e.target.value);handleChange(e) }}
                      required
                      error={error}
                      helperText={error ? "Это поле обязательно" : ""}
                      onBlur={handleChange}
                    ...
                    <TextField 
                      ...
                      type="text" 
                      value={newCath} 
                      onChange={(e)=> {setCath(e.target.value);handleChange(e) }}
                      required
                      error={error}
                      helperText={error ? "Это поле обязательно" : ""}
                      onBlur={handleChange}
                    />
                    <Input type="submit" sx={{width:100}} onClick={handleAddProduct} disabled={!isFormValid}/>
                </CardContent>
            </Box>
        </Container>
    )
